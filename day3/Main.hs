...
nextPoint (x, y) ('U':dist) = (x, y + (read dist))
nextPoint (x, y) ('R':dist) = (x + (read dist), y)
nextPoint (x, y) ('D':dist) = (x, y - (read dist))
nextPoint (x, y) ('L':dist) = (x - (read dist), y)

startPoint :: Point
startPoint = (0,0)

generate :: a -> [b] -> (a -> b -> a) -> [a]
generate start [] next = [start]
generate start (a:as) next = start : (generate (next start a) as next)

mergeAdjacent :: [(a,b)] -> ((a,b) -> (a,b) -> c) -> [c]
mergeAdjacent [] _ = []
mergeAdjacent (x:[]) _ = []
mergeAdjacent (x:y:ys) merger = (merger x y) : (mergeAdjacent (y:ys) merger)

fromInput :: [String] -> Path
fromInput strs = generate startPoint strs nextPoint

segments :: Path -> [PathSegment]
segments [] = []
segments (a:[]) = []
segments (a:b:cs) = (a,b) : (segments (b:cs))

orientation :: PathSegment -> Orientation
orientation (p1,p2)
    | (fst p1) == (fst p2) = Vertical
    | (snd p1) == (snd p2) = Horizontal

xCoords :: PathSegment -> (Int, Int)
xCoords s = (fst (fst s), fst (snd s))

yCoords :: PathSegment -> (Int, Int)
yCoords s = (snd (fst s), snd (snd s))
...
intersects :: PathSegment -> PathSegment -> Bool
intersects s1 s2
    | orientation s1 == orientation s2 = False
    | orientation s1 == Vertical = intersects s2 s1 -- now only handle case s1 Horizontal, s2 Vertical
    | otherwise =
        let s1y = snd (fst s1)
            s2x = fst (snd s2)
        in (betweenYs s2 s1y) && (betweenXs s1 s2x)

betweenYs :: PathSegment -> Int -> Bool
betweenYs seg y = ((min' (yCoords seg)) < y) && (y < (max' (yCoords seg)))

betweenXs :: PathSegment -> Int -> Bool
betweenXs seg x = ((min' (xCoords seg)) < x) && (x < (max' (xCoords seg)))

intersection :: PathSegment -> PathSegment -> Maybe Point
intersection s1 s2
    | not (intersects s1 s2) = Nothing
    | orientation s1 == Vertical = intersection s2 s1
    | otherwise = Just (fst (fst s2), snd (fst s1))
...
intersections p1 p2 = filterMaybes [intersection s1 s2 | s1 <- segments p1, s2 <- segments p2]

manhattenOrigin :: Point -> Int
manhattenOrigin (x, y) = (abs x) + (abs y)

isOnSegment :: PathSegment -> Point -> Bool
isOnSegment ps pt
    | orientation ps == Horizontal = betweenXs ps (fst pt) && snd (fst ps) == snd pt
    | otherwise = betweenYs ps (snd pt) && fst (fst ps) == fst pt

distToPoint :: PathSegment -> Point -> Int
distToPoint ps pt
    | orientation ps == Horizontal = abs ((fst (fst ps)) - (fst pt))
    | otherwise = abs ((snd (fst ps)) - (snd pt))

segLength :: PathSegment -> Int
segLength ps
    | orientation ps == Horizontal = abs ((fst (fst ps)) - fst (snd ps))
    | otherwise = abs ((snd (fst ps)) - snd (snd ps))

steps :: [PathSegment] -> Point -> Int
steps [] _ = 0
steps (x:xs) pt
    | isOnSegment x pt = distToPoint x pt
    | otherwise = (segLength x) + (steps xs pt)

stepsPerIs :: Path -> [Point] -> [Int]
stepsPerIs p is = map (steps (segments p)) is

addEach :: [(Int, Int)] -> [Int]
addEach ls = map (\x -> fst x + snd x) ls
